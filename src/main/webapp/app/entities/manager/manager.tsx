            ...
            <tbody>
              {managerList.map((manager, i) => (
                <tr key={`entity-${i}`} data-cy="entityTable">
                  ...
                  <td>{manager.civility}</td>
                  <td>{manager.fullName}</td>
                  <td>{manager.phone}</td>
                  <td>{manager.user ? manager.user.login : ''}</td>
                  <td>{manager.entreprise ? <Link to={`entreprise/${manager.entreprise.id}`}>{manager.entreprise.name}</Link> : ''}</td>
                  <td className="text-right">
                    <div className="btn-group flex-btn-group-container">
                      <Button tag={Link} to={`${match.url}/${manager.id}`} color="info" size="sm" data-cy="entityDetailsButton">
                        <FontAwesomeIcon icon="eye" /> <span className="d-none d-md-inline">View</span>
                      </Button>
                      <Button tag={Link} to={`${match.url}/${manager.id}/edit`} color="primary" size="sm" data-cy="entityEditButton">
                        <FontAwesomeIcon icon="pencil-alt" /> <span className="d-none d-md-inline">Edit</span>
                      </Button>
                      <Button tag={Link} to={`${match.url}/${manager.id}/delete`} color="danger" size="sm" data-cy="entityDeleteButton">
